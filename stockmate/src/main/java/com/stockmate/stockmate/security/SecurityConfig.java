 ...
 *
 * ┌─────────────────────────────────────────────────────────────────────────┐ │
 * TWO-LAYER SECURITY MODEL (CLAUDE.md + PRD FR-AUTH-06/08/09) │ │ │ │ LAYER 1 —
 * URL level (this class) │ │ Controls which HTTP paths are reachable by which
 * roles. │ │ e.g. /admin/** → ADMIN only; /orders/my → BUYER only │ │ │ │ LAYER
 * 2 — Method level (@PreAuthorize in ServiceImpl classes) │ │ Controls
 * business-rule ownership inside a role. │ │ e.g. SELLER can only edit THEIR
 * OWN product (URL-level can't do this)│ │ │ │ BOTH layers are required — layer
 * 1 alone is insufficient per PRD. │
 * └─────────────────────────────────────────────────────────────────────────┘
 *
 * MIGRATION NOTE — Spring Security 6.5+ ──────────────────────────────────────
 * AntPathRequestMatcher is DEPRECATED and marked for removal. Replacement:
 * PathPatternRequestMatcher Package:
 ...
 *
 * BEANS PRODUCED ────────────── PasswordEncoder BCrypt(12) — injected into
 * UserServiceImpl DaoAuthenticationProvider links CustomUserDetailsService +
 * PasswordEncoder AuthenticationManager needed by AuthController for
 * programmatic login SecurityFilterChain URL rules, form-login, logout,
 * session, 403 page
 */
@Configuration
@EnableWebSecurity

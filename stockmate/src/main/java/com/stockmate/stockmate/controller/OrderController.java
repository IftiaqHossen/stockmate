 ...
 * ══════════════════════════════════════════════════════════════════════════
 *   ✅ Maps URLs → OrderService calls
 *   ✅ Extracts username from SecurityContext via @AuthenticationPrincipal
 *   ✅ Passes validated DTOs to OrderService
 *   ❌ No business logic — stock validation, decrement, ownership → service layer
 ...
 *     GET  /orders              → ADMIN
 *     PUT  /orders/{id}/status  → SELLER or ADMIN
 *     PUT  /orders/{id}/cancel  → BUYER
 *   Layer 2 (@PreAuthorize in OrderServiceImpl):
 *     Ownership checks — BUYER can only cancel their own order,
 *     SELLER can only update status for their own product's orders.
 *
 * ENDPOINT SUMMARY
 * ────────────────────────────────────────────────────────────────
 *   POST /orders                 → place order (BUYER)         FR-ORD-01/02/03
 *   PUT  /orders/{id}/cancel     → cancel order (BUYER)        FR-ORD-01
 *   GET  /orders/my              → buyer's order history       FR-ORD-04
 *   GET  /orders/seller          → seller's incoming orders    FR-ORD-05
 *   GET  /orders                 → all orders (ADMIN)          FR-ORD-06
 *   PUT  /orders/{id}/status     → update status (SELLER/ADM)  FR-ORD-07
 *
 * HTML FORM NOTE:
 *   PUT uses Spring's HiddenHttpMethodFilter.
...
public class OrderController {
     ...
     * Places a new order.
     *
     * The form lives on the product detail page (/products/{id}).
     * On success → redirect to /orders/my (buyer's order list).
     *
     * InsufficientStockException → GlobalExceptionHandler → 409 page.
     * ResourceNotFoundException  → GlobalExceptionHandler → 404 page.
     *
     * BindingResult errors (e.g. quantity < 1) redirect back to the
     * product detail page with a flash error.
     */
    @PostMapping
     ...
     *   → GlobalExceptionHandler → 403 page.
     * IllegalStateException (order already DELIVERED or CANCELLED)
     *   → caught here for a friendly flash message (better UX than an error page).
     */
    ...
    /**
     * Shows the buyer's personal order history, newest first.
     * URL-level guard in SecurityConfig restricts this to BUYER.
     */
    @GetMapping("/my")
     ...
     * URL-level guard in SecurityConfig restricts GET /orders to ADMIN.
     */
    @GetMapping
     ...
     *
     * Ownership check for SELLER (may only update their product's orders)
     * lives in OrderServiceImpl — AccessDeniedException → GlobalExceptionHandler → 403.
     *
     * Terminal state guard (DELIVERED / CANCELLED) caught here for friendly flash.
     *
     * After update, SELLER is redirected to /orders/seller,
     * ADMIN is redirected to /orders.
     */
    ...
    /**
     * After a status update, redirect SELLER to /orders/seller
     * and ADMIN to /orders — each role returns to their own view.
     */
